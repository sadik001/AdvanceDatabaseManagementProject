...
if (!empty($name) || !empty($email) || !empty($pass) || !empty($gender) || !empty($dob) || !empty($phone)) 
    {
      $username = $_SESSION['username'];
           ...
           while(($user_fetch = oci_fetch_array($stid1, OCI_BOTH)) != false)
            ...
            $stid = oci_parse($con, $queryU);
            $resultU=oci_execute($stid);
             // while (($row = oci_fetch_array($stid, OCI_BOTH)) != false) {
             //  $id=$row['ID'];
             // }
            
            $rowsU = oci_num_rows($resultU);

            $queryME = "SELECT Email FROM user_data WHERE Username = '$username'";
            
            $rowsME = mysqli_num_rows($resultME);
            $stid = oci_parse($con, $queryME);
            $resultsME = oci_execute($stid);
            $rowsME = oci_num_rows($resultME);
         ...
         if ($rowsU == 1)
         {
            ...
            $stid = oci_parse($con, $queryE);
            $resultE=oci_execute($stid);
             // while (($row = oci_fetch_array($stid, OCI_BOTH)) != false) {
             //  $id=$row['ID'];
             // }
            
            $rowsE = oci_num_rows($resultE);
